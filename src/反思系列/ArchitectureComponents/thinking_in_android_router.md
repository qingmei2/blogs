...
在阅读[这篇文章](https://developer.aliyun.com/article/71687)时，笔者了解到了这个功能的实现历程，很有趣的是，在该功能最初的实现方案中，是运行期通过反射拿到`ActivityThread`实例，最终在`Activity`实例化的时候，通过反射把`Intent`预先存好的参数值写入到需要自动装配的字段中实现的。

这种方案的缺点很明显，除了反射带来的性能影响外，甚至可能导致用户的代码出现`NPE`，因此这种实现方式后来被新的方案所代替。

新的方案依然是我们的老朋友`AnnotationProcessor`，在编译期间，为`Activity`本身生成一个对应的注入辅助类，运行时通过辅助类对`Activity`中的字段进行赋值——这不由让人想起了`Square`的`dagger`，被`Google`后来居上的`dagger2`代替的情况。

那么，为什么其它通信库没有像`ARouter`一样提供这样一个依赖注入的功能呢，是因为做不到吗？

并非如此，在其它通信库中，我们更建议，将页面的跳转进行了更高维度的抽象，因此，单纯为页面的跳转设计依赖注入是意义不大的，如果设计一个新的功能，这个功能也应该是针对完整的 **通信本身** ，而非部分的细节。
